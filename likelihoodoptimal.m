function [NLML,D_NLML]=likelihoodoptimal(hyp)
...
K_fu = K_uf';
K_ff = k_ff(x_f, x_f, hyp(1:end),0);% + eye(n_f).*sigma_n_f;
...
for i=1:D+1
    DKuu = k_uu(x_u, x_u, hyp(1:D+1),i);
    DKuf = k_uf(x_u, x_f, hyp(1:end),i);
    DKfu = DKuf';
    DKff = k_ff(x_f, x_f, hyp(1:end),i);

    DK = [DKuu DKuf;
        DKfu DKff];

    D_NLML(i) = sum(sum(Q.*DK))/2;
    for i=D+2:length(hyp)
    DK_uu = zeros(n_u,n_u);
    DK_uf = k_uf(x_u, x_f, hyp(1:end),i);
    DK_fu = DK_uf';
    DK_ff = k_ff(x_f, x_f, hyp(1:end),i);
