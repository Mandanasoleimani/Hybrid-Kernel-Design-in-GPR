...
noise=sigma_n*I;
K1=exp(-ep^2*DM.^2);
K2=exp(-ep*DM).*(1+(ep.*DM)+(1/3).*((ep.*DM).^2));
...
Ky2=K1;
Ky3=K2+noise;  % Compute covariance matrix of training data
Ky4=K2;
if i==1
condKnoisefree(i,j)=condest(Ky2);
condKnoisefree2(i,j)=condest(Ky4);
end
condKnoisy(i,j)=condest(Ky1);
condKnoisy2(i,j)=condest(Ky3);
end
end
xxx=X(1,:);
yyy=Y(:,1);
figure
hold on
plot(log10(yyy),log10(condKnoisy(:,1)),'b')
plot(log10(yyy),log10(condKnoisy2(:,1)),'r')
xlabel('\sigma')
ylabel('log_{10}(Cond(K) with noise for \epsilon= 0.5)')
legend('condition number for Gaussian and Matern 3/2 kernels with different \sigma','Location','southwest')
figure
hold on
plot(log10(yyy),log10(condKnoisy(:,2)),'b')
plot(log10(yyy),log10(condKnoisy2(:,2)),'r')
xlabel('\sigma')
ylabel('log_{10}(Cond(K) with noise for \epsilon= 1)')
legend('condition number for Gaussian and Matern 3/2 kernels with different \sigma','Location','southwest')
figure
hold on
plot(log10(yyy),log10(condKnoisy(:,3)),'b')
plot(log10(yyy),log10(condKnoisy2(:,3)),'r')
xlabel('\sigma')
ylabel('log_{10}(Cond(K) with noise for \epsilon= 1.5)')
legend('condition number for Gaussian and Matern 3/2 kernels with different \sigma','Location','southwest')
...
% %legend('condition number for Gaussian kernel with different \epsilon','Location','southwest')
% legend('condition number for Gaussian and Matern 3/2 kernels with different \epsilon','Location','southwest')
%%%%
% hold on
% plot(xxx,log10(condKnoisefree2))
% xlabel('\epsilon')
% ylabel('log_{10}(Cond(K) noise free)')
% %legend('condition number for Gaussian kernel with different \epsilon','Location','southwest')
% hold off
...
hold on
%surf(X,log10(Y),log10(condKnoisy2))
xlabel('\epsilon'),ylabel('log_{10}(\sigma)'),zlabel('log_{10}(Cond(K) with noise)')
legend('condition number for Gaussian and Matern 3/2 kernels with different \epsilon and noise','Location','southwest')
